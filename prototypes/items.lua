data:extend({
    {
        type = "item-with-entity-data",
        name = "kj_stargate_iris",
        icon = "__kj_stargate__/graphics/entities/stargate/iris_icon.png",
        icon_size = 128,
        subgroup = "transport",
        order = "1",
        --inventory_move_sound = item_sounds.vehicle_inventory_move,
        --pick_sound = item_sounds.vehicle_inventory_pickup,
        --drop_sound = item_sounds.vehicle_inventory_move,
        stack_size = 1,
        weight = 1000 * 1000,
    },
    {
        type = "item-with-entity-data",
        name = "kj_stargate",
        icon = "__kj_stargate__/graphics/entities/stargate/stargate_icon.png",
        icon_size = 128,
        subgroup = "transport",
        order = "0",
        --inventory_move_sound = item_sounds.vehicle_inventory_move,
        --pick_sound = item_sounds.vehicle_inventory_pickup,
        --drop_sound = item_sounds.vehicle_inventory_move,
        stack_size = 1,
        weight = 1000 * 1000,
    },
    {
        type = "item-with-entity-data",
        name = "kj_stargate_placement",
        icon = "__kj_stargate__/graphics/entities/stargate/icon.png",
        icon_size = 128,
        subgroup = "transport",
        order = "2",
        --inventory_move_sound = item_sounds.vehicle_inventory_move,
        --pick_sound = item_sounds.vehicle_inventory_pickup,
        --drop_sound = item_sounds.vehicle_inventory_move,
        place_result = "kj_stargate_placement",
        stack_size = 1,
        weight = 10000 * 1000,
    },
    {
        type = "item-with-entity-data",
        name = "kj_stargate_signaled_placement",
        icon = "__kj_stargate__/graphics/entities/stargate/s_icon.png",
        icon_size = 128,
        subgroup = "transport",
        order = "3",
        --inventory_move_sound = item_sounds.vehicle_inventory_move,
        --pick_sound = item_sounds.vehicle_inventory_pickup,
        --drop_sound = item_sounds.vehicle_inventory_move,
        place_result = "kj_stargate_signaled_placement",
        stack_size = 1,
        weight = 10000 * 1000,
    },
    {
        type = "item-with-entity-data",
        name = "kj_stargate_signaled_iris_placement",
        icon = "__kj_stargate__/graphics/entities/stargate/s_gate_iris_icon.png",
        icon_size = 128,
        subgroup = "transport",
        order = "4",
        --inventory_move_sound = item_sounds.vehicle_inventory_move,
        --pick_sound = item_sounds.vehicle_inventory_pickup,
        --drop_sound = item_sounds.vehicle_inventory_move,
        place_result = "kj_stargate_signaled_iris_placement",
        stack_size = 1,
        weight = 10000 * 1000,
    },
    {
        type = "item-with-entity-data",
        name = "kj_dhd",
        icon = "__kj_stargate__/graphics/entities/dhd/icon.png",
        icon_size = 128,
        subgroup = "transport",
        order = "0",
        --inventory_move_sound = item_sounds.vehicle_inventory_move,
        --pick_sound = item_sounds.vehicle_inventory_pickup,
        --drop_sound = item_sounds.vehicle_inventory_move,
        place_result = "kj_dhd",
        stack_size = 1,
        weight = 1000 * 1000,
    },
})